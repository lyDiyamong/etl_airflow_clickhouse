            ...
            for subject_detail in subject_details:
                try:
                    ...
                    if isinstance(subject_detail, dict):
                        ...
                        semester_start_date = subject_detail.get('semesterStartDate')
                        ...
                        semester_start_date = subject_detail[18]
    ...
    for (class_id, class_name, subject_name), data in class_subject_data.items():
        ...
        for point in monthly_data:
            if point['semester_name'] and point['semester_eval_id']:
                # Include semester_start_date in the key to ensure proper grouping
                key = (point['semester_name'], point['semester_eval_id'], point['semester_start_date'])
                semester_groups[key].append(point)
        
        # Calculate semester details
        semester_details = []
        for (semester_name, semester_id, semester_start_date), scores in semester_groups.items():
            # Now semester_start_date is already part of the key, so we don't need to extract it from scores
            
            # Separate regular monthly scores from final exam scores
            regular_months = [s for s in scores if s.get('month_name', '').lower() != 'final exam']
            final_exam = [s for s in scores if s.get('month_name', '').lower() == 'final exam']
            
            # Calculate average of regular monthly scores
            if regular_months:
                avg_score = sum(s['score'] for s in regular_months) / len(regular_months)
            else:
                avg_score = 0
                
            # Get the final exam score if available
            if final_exam:
                exam_score = sum([s['score'] for s in final_exam]) / len(final_exam)
            else:
                # Fallback if no final exam score is found
                exam_score = 0
            
            # Calculate final score (average of monthly average and exam)
            if avg_score > 0 or exam_score > 0:
                final_score = (avg_score + exam_score) / 2
            else:
                final_score = 0
            ...
            semester_details.append((
                semester_id, 
                semester_name, 
                semester_start_date,  # This is now correctly associated with this specific semester
                round(avg_score, 2), 
                round(exam_score, 2), 
                round(final_score, 2)
            ))
        ...
        yearly_score = 0
        if semester_details:
            yearly_score = sum(s[5] for s in semester_details) / len(semester_details)
        ...
        if key == 'monthDetails':
            ...
            for tup in value:
                ...
                for i, elem in enumerate(tup):
                    ...
                    elif i == 4 and elem is not None:  # semesterStartDate (Date)
                        elements.append(f"'{elem}'")
            ...
            for tup in value:
                ...
                for i, elem in enumerate(tup):
                    if elem is None:
                        ...
                        elements.append(f"'{elem}'")
                    elif i == 2 and elem is not None:  # semesterStartDate (Date)
                        elements.append(f"'{elem}'")
                    elif isinstance(elem, str):
                        escaped = elem.replace("'", "''")
                        elements.append(f"'{escaped}'")
